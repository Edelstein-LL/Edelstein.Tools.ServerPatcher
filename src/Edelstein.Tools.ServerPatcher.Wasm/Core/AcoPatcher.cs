using Edelstein.Tools.ServerPatcher.Wasm.Encryption;
...
public static class AcoPatcher
{
    private static readonly JsonSerializerOptions IndentedJsonSerializerOptions = new() { WriteIndented = true };

    public static async Task Patch(IJSRuntime jsRuntime, Stream acoAssetStream, string apiUriString, string assetsUriString, GameRegion gameRegion)
    {
        ...
        await ManifestCryptor.DecryptUncompressedAsync(jsRuntime, encryptedAcoStream, decryptedAcoStream);
        ...
        await ManifestCryptor.EncryptUncompressedAsync(jsRuntime, decryptedAcoStream, encryptedAcoStream);
        encryptedAcoStream.Seek(0, SeekOrigin.Begin);
